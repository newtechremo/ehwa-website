...
interface A11yBarProps {
  ...
  fontScale: number
  onToggleLowVision: () => void
  onToggleSignLanguage: () => void
  onChangeFontSize: (direction: 1 | -1) => void
}
...
export function A11yBar({
  ...
  return (
    <nav
      className="sticky top-0 z-[2000] bg-[#222] text-white py-2.5 border-b border-[#444]"
      aria-label="접근성 도구"
    >
      <div className="max-w-[1200px] mx-auto px-5 flex justify-between items-center">
        ...
        <div className="flex gap-2.5 items-center">
          {/* 1. 고대비 버튼 */}
          <button
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded text-sm font-medium border transition-all whitespace-nowrap
              ${
                lowVision
                  ? "bg-white text-black border-white font-bold"
                  : "bg-[#444] text-white border-[#666] hover:bg-[#555]"
              }`}
            onClick={onToggleLowVision}
          ...
          {/* 2. 글자크기 조절 그룹 */}
          <div
            className="flex items-center bg-[#444] border border-[#666] rounded overflow-hidden"
            role="group"
            aria-label="글자 크기 조정"
          >
            <button
              className="px-3 py-1.5 border-r border-[#666] hover:bg-[#555] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              onClick={() => onChangeFontSize(-1)}
              disabled={lowVision || fontScale === 100}
              aria-label="글자 크기 축소"
              aria-disabled={lowVision || fontScale === 100}
            >
              <Minus className="h-4 w-4" />
            </button>
            <span className="px-2.5 text-xs font-semibold text-[#ddd] hidden md:flex items-center gap-1">
              <i className="fa-solid fa-text-height" aria-hidden="true"></i> 글자
            </span>
            <button
              className="px-3 py-1.5 hover:bg-[#555] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              onClick={() => onChangeFontSize(1)}
              disabled={lowVision || fontScale === 150}
              aria-label="글자 크기 확대"
              aria-disabled={lowVision || fontScale === 150}
            >
